...
class CustomUserFilter(FilterPlaceholderSetup, django_filters.FilterSet):
    ...
    email = django_filters.CharFilter(label=_("Email"), field_name="email", lookup_expr="icontains")
    role = django_filters.ChoiceFilter(label=_("Role"), choices=CustomUserRole.choices)
    is_active = django_filters.ChoiceFilter(label=_("Active"), choices=BOOLEAN_CHOICES)
