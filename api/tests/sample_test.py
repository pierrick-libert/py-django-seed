...
class SampleTests(APITestCase):
    ...
    def test_sample(self) -> None:
        ...
        response = self.client.post(
            "/api/sample/non-generic", data={"test": "test"}, headers=self.headers, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        # POST ERROR /sample/non-generic
        response = self.client.post("/api/sample/non-generic", data={}, headers=self.headers, format="json")
